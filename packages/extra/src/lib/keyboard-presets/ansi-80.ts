...
export function keyboardFormFactorANSI80(kbd: Keyboard): Layout {
  ...
  const gap = function (unitSize: number): Layout {
    return new Layout({
      width: kbd.unitToSize(unitSize),
    });
  };

  const keyRow = function (
    keyDefs: (number | Node)[],
    extraProps?: LayoutProps,
  ): Layout {
    return new Layout({
      gap: keyGap,
      ...
      children: {
        ...keyDefs.map(keyDef =>
          typeof keyDef === 'number' ? kbd.createKey(keyDef) : keyDef,
        ),
      },
      ...extraProps,
    });
  ...
  const layoutScheme = new Layout({
    ...
    children: {
      ...[
        keyRow(
          [1, gap(1), 1, 1, 1, 1, gap(0.5), 1, 1, 1, 1, gap(0.5), 1, 1, 1, 1],
          {marginBottom: 25},
        ), // function key row
...
export function keyboardLayoutQWERTYforANSI80(): KeyboardLayout {
  return [
    ...
    k2(':', ';'),
    k2('"', "'"),
    sk('Enter', ['Enter', 'Return']),
    sk('Shift', ['LShift', 'Shift']),
    k('Z'),
    k('X'),
    k('C'),
    k('V'),

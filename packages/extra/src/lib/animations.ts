import {Curve, FlexDirection, Layout} from '@motion-canvas/2d';
import {
  DEFAULT,
  Vector2,
  all,
...
export function* nop(durationOrEventName: number | string = DEFAULT_DURATION) {
  const duration =
    typeof durationOrEventName === 'number'
      ? durationOrEventName
...
export function* smoothInsert(
  ...
  layout.insert(tempContainer, index);
...
export function* smoothInsertAlt(
  layout: Layout,
  node: Layout,
  index: number = 0,
  duration: number = DEFAULT_DURATION,
...
export function* smoothRemove(
  ...
  layout.insert(tempContainer, tempIndex);
...
export function* smoothRemoveAlt(
  layout: Layout,
  node: Layout,
  duration: number = DEFAULT_DURATION,
) {
...
export function* smoothAddWithAutoscroll(
  layout: Layout,
  node: Layout,
  duration: number = DEFAULT_DURATION,
) {
  ...
  const scrollDirections: Record<FlexDirection, Vector2> = {
    ['column']: new Vector2(
      0,
      -sizeDiff.y * normalize(layout.offset().y, 1, -1),
    ),
    ['column-reverse']: new Vector2(
      0,
      sizeDiff.y * normalize(layout.offset().y, -1, 1),
    ),
    ['row']: new Vector2(-sizeDiff.x * normalize(layout.offset().x, 1, -1), 0),
    ['row-reverse']: new Vector2(
      sizeDiff.x * normalize(layout.offset().x, -1, 1),
      0,
    ),
  };

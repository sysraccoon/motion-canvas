...
export class Directory extends Layout {
  ...
  private readonly dirIcon: SimpleSignal<string, this> = createSignal(
    icons.openDirectory,
  );
  ...
  public *open(
    duration: number = 0.8,
    wavePosition: Vector2 = Vector2.zero,
  ): ThreadGenerator {
    const originalAccent = this.dirNode().fill.context.raw();
    yield* all(
      chain(
        this.dirNode().waveChangeAccent(
          colors.background,
          duration * 0.5,
          wavePosition,
        ),
        this.dirNode().fill(
          originalAccent ?? DEFAULT,
          duration * 0.5,
          easeInCubic,
        ),
      ),

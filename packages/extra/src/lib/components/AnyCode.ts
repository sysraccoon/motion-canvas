...
export class AnyCode extends Code {
  ...
  private extract(
    ...
    const rawCleaned = this.concatenateStringsAndTags(
      cleanedStrings as unknown as TemplateStringsArray,
      cleanedTags as RawCodeFragment[],
    );

    return {
      raw,
      rawCleaned,
      strings: cleanedStrings as unknown as TemplateStringsArray,
      tags: cleanedTags,
    };
  }

  private concatenateStringsAndTags(
    strings: TemplateStringsArray,
    tags: RawCodeFragment[],
  ): string {
    return strings.reduce(
      (acc, str, index) => acc + (index > 0 ? tags[index - 1].after : '') + str,
      '',
    );
  }
...
export class ManualHighlighter
  implements CodeHighlighter<ManualHiCache | null>
{

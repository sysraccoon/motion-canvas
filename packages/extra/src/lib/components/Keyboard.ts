import {Layout, RectProps, initial, signal} from '@motion-canvas/2d';
import {
  Color,
  ColorSignal,
  PossibleColor,
...
import {Key, KeyLabels, KeyProps} from './Key';
...
export class Keyboard extends Layout {
  ...
  public *seqCombo(keys: Key[], holdDuration: number = 1.5): ThreadGenerator {
    ...
    for (const key of keys) {
      holdTasks.push(chain(waitFor(totalDelay), key.press()));
  ...
  public setKeyboardLayout(layer: KeyboardLayout) {
    if (this.rawKeys.length !== layer.length) {
      throw Error(
        `raw keys and new labels not the same size (rawKeys = ${this.rawKeys.length}, labels = ${layer.length})`,
      );
    }
  ...
  public *tileInTransition(
    basePosition?: Vector2,
    delayDuration: number = 0.4,
    inDuration: number = 0.6,
  ) {
  ...
  public *tileOutTransition(
    basePosition?: Vector2,
    delayDuration: number = 0.4,
    outDuration: number = 0.6,
  ) {
    basePosition = basePosition ?? this.absolutePosition();
  ...
  private searchDistancesForKey(basePosition: Vector2): Map<Key, number> {
    ...
    for (const key of this.rawKeys) {
      const currentDistance = key
        .absolutePosition()
        .sub(basePosition).squaredMagnitude;
